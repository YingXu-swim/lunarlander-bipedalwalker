...
@hydra.main(config_path='configs/', config_name='lunarlander_discrete_easy_dqn')
def main(cfg):
    # set random seed
    h.set_seed(cfg.seed)
    ...
    env = create_env(config_file_name='./env/lunarlander_discrete_easy', seed=cfg.seed)
    ...
    if cfg.agent_name == "dqn":
        agent = DQNAgent(state_shape=state_shape, n_actions=n_actions, batch_size=cfg.batch_size, hidden_dims=cfg.hidden_dims,
    ...
    if not cfg.testing:# training
        ...
        for ep in range(cfg.train_episodes):
            ...
            if cfg.save_model and ep in [1,300, 1000, 1300, 1500]: agent.save(model_path, cfg.seed, ep)

        # save model and logging    
        if cfg.save_model:
            agent.save(model_path, cfg.seed, ep)
        if cfg.save_logging:
        ...
        agent.load(model_path, cfg.seed, 1999)
        ...
        print("Average test reward:", total_test_reward/cfg.num_episode)
