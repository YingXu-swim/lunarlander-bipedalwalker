...
class DQNAgent(object):
    ...
    def save(self, fp, seed, ep):
        str_ = 'dqn' + str(seed)+'_'+str(ep)+'.pt'
        path = fp/str_
        torch.save({
    ...
    def load(self, fp, seed, ep):
        str_ = 'dqn' + str(seed)+'_'+str(ep)+'.pt'
        path = fp/str_
        d = torch.load(path)
        self.policy_net.load_state_dict(d['policy'], strict=True)
        self.target_net.load_state_dict(d['policy_target'], strict=True)