...
class DDPG(object):
    ...
    def _update(self,):
        ...
        action = self.pi(batch.state)
        action_ran = self.pi_target(batch.next_state)
        q_val = self.q_target(batch.next_state, action_ran)
        
        q_tar = batch.reward + (self.gamma * batch.not_done * q_val).detach()
        q_eval = self.q(batch.state, batch.action)      
        
        # 2. compute the critic loss and update the q's parameters
        critic_loss = F.mse_loss(q_eval, q_tar)
        ...
        actor_loss = torch.mean(-self.q(batch.state, action))
    ...
    def get_action(self, observation, evaluation=False):
        ...
        action = self.pi(x)
        if evaluation == True:
            noise = torch.distributions.Normal(0, expl_noise).sample()
            action = action + noise
